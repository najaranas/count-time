...
function addColorToLocalStorage(color) {
    window.localStorage.setItem("settings-color", color);
}
if (window.localStorage.getItem("settings-color")) {
    ...
    colorBoxes.forEach((colorBox) => {
        if (colorBox.getAttribute("data-colors") === localColor) {
            colorBox.classList.add("checked");
            colorBox.style.borderColor = localColor;
            colorBox.style.opacity = "1";
        }
    });
}
